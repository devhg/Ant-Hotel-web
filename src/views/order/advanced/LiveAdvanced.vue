...
<template>
  ...
  >
    <detail-list slot="headerContent" size="small" :col="2" class="detail-layout">
      <detail-list-item term="创建人">{{info.uname}}</detail-list-item>
      <detail-list-item term="手机号">{{info.phoneNumber}}</detail-list-item>
      <detail-list-item term="创建时间">{{format(info.reserveTime)}}</detail-list-item>
      <detail-list-item term="关联单据">
        <a>12421</a>
      </detail-list-item>
      <detail-list-item term="生效日期">{{format(info.startTime)}} ~ {{format(info.endTime)}}</detail-list-item>
      <detail-list-item term="备注">无</detail-list-item>
    </detail-list>
    <a-row slot="extra" class="status-list">
      <a-col :xs="12" :sm="12">
        <div class="text">订单金额</div>
        <div class="heading">¥ {{info.preFee}}</div>
      </a-col>
    </a-row>
    <!-- actions -->
    ...
    <a-card style="margin-top: 24px" :bordered="false" title="用户信息">
      <detail-list>
        <detail-list-item term="用户姓名">{{info.uname}}</detail-list-item>
    ...
    <a-card style="margin-top: 24px" :bordered="false" title="用户近半年住宿记录">
      <div class="no-data">
...
export default {
  ...
  methods: {
    getData() {
      getOrderList(this.$route.query).then(res => {
        this.info = res.data
        console.log(res)
      })
    },
